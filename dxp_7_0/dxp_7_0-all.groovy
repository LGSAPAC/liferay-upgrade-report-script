...
// Search Information
// There's no SearchEngineInformation OSGi service in 7.0.10-ga1
...
searchEnginesIds.each { searchEnginesById[it] = SearchEngineHelperUtil.getSearchEngine(it) }

def searchEngine = SearchEngineHelperUtil.getSearchEngine(SearchEngineHelperUtil.getDefaultSearchEngineId()).getVendor()
def searchEngineDetails = """\
Via SearchEngineHelperUtil::getSearchEngineIds:
...
// the function 'invoke__1_infra_groovy' will be invoked at the very end of the 'dxp_7_0-all.groovy'
...
// TODO this does not seem to work for 7.0 -- always returns 0 pages
def liferayPortlets =
...
// the function 'invoke__2_data_groovy' will be invoked at the very end of the 'dxp_7_0-all.groovy'
...
// the function 'invoke__3_database_info_groovy' will be invoked at the very end of the 'dxp_7_0-all.groovy'
...
// 'uasContext' is a parameter to the wrapper function for this script when combining it with the others;
    ...
    // PropsValues: Constants are not present in PropsValues in 7.0-ga1 (even the keys are not in its portal.properties),
    // only some FP later
    //  => check prop is set, if not, then use the commonly used constant value
    // Source:
    //  * https://docs.liferay.com/ce/portal/7.0-latest/propertiesdoc/portal.properties.html
    //  * https://github.com/liferay/liferay-portal/blob/7.0.0-ga1/portal-impl/src/portal.properties#L6407-L6411
    String _PROPS_VALUES_MODULE_FRAMEWORK_MARKETPLACE_DIR =
            PropsUtil.get('module.framework.marketplace.dir') ?:
                    new File(PropsValues.MODULE_FRAMEWORK_BASE_DIR, 'marketplace').path
    String _PROPS_VALUES_MODULE_FRAMEWORK_MODULES_DIR =
            PropsUtil.get('module.framework.modules.dir') ?:
                    new File(PropsValues.MODULE_FRAMEWORK_BASE_DIR, 'modules').path
    String _PROPS_VALUES_MODULE_FRAMEWORK_WAR_DIR =
            PropsUtil.get('module.framework.war.dir') ?:
                    new File(PropsValues.MODULE_FRAMEWORK_BASE_DIR, 'war').path
...
// the function 'invoke__4_custom_code_groovy' will be invoked at the very end of the 'dxp_7_0-all.groovy'
